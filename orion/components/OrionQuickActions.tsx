      ...
      <div className="flex max-h-20 flex-wrap gap-2 overflow-y-auto pb-1 [scrollbar-width:thin]">
        {actions.slice(0, 4).map((action, index) => (
          <motion.button
            ...
            className="min-w-0 rounded-full border border-[#BFDBFE] bg-[#EFF6FF] px-3 py-2 text-xs font-semibold text-[#2563EB] transition hover:border-[#60A5FA] hover:bg-[#DBEAFE]"
          >
            {action.label}
          </motion.button>
        ))}
      ...
      {actions.map((action, index) => {
        ...
        return (
          <motion.button
            ...
            whileHover={{ y: -1 }}
            onClick={() => onAction(action.id)}
            className="min-w-0 rounded-full border border-[#BFDBFE] bg-white px-3 py-2 text-left text-[#2563EB] transition hover:border-[#60A5FA] hover:bg-[#EFF6FF]"
          >
            <div className="flex items-center gap-2">
              <Icon className="h-3.5 w-3.5 shrink-0" />
              <div className="min-w-0">
                <div className="truncate text-xs font-semibold">{action.label}</div>
              </div>
            </div>
          </motion.button>
        );

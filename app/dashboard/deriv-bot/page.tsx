...
export default function DerivBotPage() {
  ...
  const autoScanAccountRef = useRef('');
  ...
  useEffect(() => {
    if (!token || !selectedAccount || !ticks.length || autoScanAccountRef.current === selectedAccount) return;
    autoScanAccountRef.current = selectedAccount;
    void runScan();
  }, [selectedAccount, ticks.length, token]);
  ...
  const placeTrade = async (contractType: 'DIGITMATCH' | 'DIGITDIFF') => {
    if (!token || !selectedAccount) return;
    const digit = selectedDigit;
    setTrading(true);
    setError('');
    try {
      const result = await api.derivBot.burstTrade({ accountId: selectedAccount, contractType, digit, stake: Number(stake), duration: 1, totalTrades: Number(totalTrades) }, token);
      if (result.executedTrades < result.requestedTrades) setError(`Executed ${result.executedTrades} of ${result.requestedTrades} requested trades because the available balance was reached.`);
  ...
  const digitCounts = Array.from({ length: 10 }, (_, digit) => ticks.filter((tick) => getTickDigit(tick, symbol) === digit).length);
  const digitSampleSize = Math.max(ticks.length, 1);
  const visibleAccounts = accountTypeFilter === 'all' ? accounts : accounts.filter((account) => account.accountType === accountTypeFilter);
    ...
    <main className="mx-auto max-w-6xl space-y-5 bg-[#070d1b] px-3 py-5 text-slate-100 sm:px-6 lg:px-8">
      ...
      {!accounts.length ? (
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-800 bg-[#0d1728] p-4"><div className="grid gap-3 lg:grid-cols-[auto_auto_minmax(220px,1fr)_auto] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Account type</p><div className="mt-2 flex rounded-lg border border-slate-700 bg-[#091221] p-1"><button type="button" onClick={() => { setAccountTypeFilter('all'); setSelectedAccount(accounts[0]?.id || ''); }} className={`rounded-md px-3 py-2 text-xs font-bold ${accountTypeFilter === 'all' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400'}`}>All</button><button type="button" onClick={() => { const next = accounts.find((account) => account.accountType === 'demo'); setAccountTypeFilter('demo'); setSelectedAccount(next?.id || ''); }} className={`rounded-md px-3 py-2 text-xs font-bold ${accountTypeFilter === 'demo' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400'}`}>Demo</button><button type="button" onClick={() => { const next = accounts.find((account) => account.accountType === 'real'); setAccountTypeFilter('real'); setSelectedAccount(next?.id || ''); }} className={`rounded-md px-3 py-2 text-xs font-bold ${accountTypeFilter === 'real' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400'}`}>Real</button></div></div><label className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Account<select value={visibleAccounts.some((account) => account.id === selectedAccount) ? selectedAccount : ''} onChange={(event) => setSelectedAccount(event.target.value)} className="mt-2 h-11 w-full min-w-0 rounded-lg border border-slate-700 bg-[#091221] px-3 text-sm font-semibold text-white lg:min-w-[260px]"><option value="" disabled>Select account</option>{visibleAccounts.map((account) => <option key={account.id} value={account.id}>{account.accountType.toUpperCase()} - {account.maskedAccountId} ({account.currency})</option>)}</select></label><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">{selectedAccountRecord?.accountType || 'Deriv'} balance</p><p className="mt-2 text-2xl font-black text-white">{liveBalance == null ? 'Connecting...' : `${selectedAccountRecord?.currency || ''} ${liveBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}</p></div><Button variant="outline" onClick={disconnect} className="gap-2 border-slate-700 bg-slate-900 text-slate-200 hover:bg-slate-800"><Unplug className="h-4 w-4" />Disconnect</Button></div></div>
          <Card className="border-slate-800 bg-[#0d1728] text-white"><CardHeader><CardTitle className="text-white">Trading terminal</CardTitle></CardHeader><CardContent className="space-y-5">
            <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto_auto] sm:items-end"><div><p className="text-sm font-semibold text-slate-300">Market</p><Button type="button" onClick={() => setMarketPickerOpen(true)} className="mt-2 flex h-11 w-full items-center justify-between border border-slate-700 bg-[#091221] px-3 font-mono text-white hover:bg-slate-800 sm:min-w-[300px]"><span>{DERIV_INDICES.find((index) => index.symbol === symbol)?.label ?? symbol} <span className="ml-2 text-slate-500">({symbol})</span></span><ChevronDown className="h-4 w-4 text-cyan-300" /></Button></div><label className="text-sm font-semibold text-slate-300">Stake<input value={stake} onChange={(event) => setStake(event.target.value)} inputMode="decimal" className="mt-2 h-11 w-28 rounded-lg border border-slate-700 bg-[#091221] px-3 text-white" /></label><label className="text-sm font-semibold text-slate-300">Total trades<input value={totalTrades} onChange={(event) => setTotalTrades(event.target.value.replace(/\D/g, '').slice(0, 3))} inputMode="numeric" min="1" max="100" className="mt-2 h-11 w-32 rounded-lg border border-slate-700 bg-[#091221] px-3 text-white" /></label><Button onClick={runScan} disabled={scanning || !ticks.length} className="h-11 gap-2 bg-cyan-500 text-slate-950 hover:bg-cyan-400">{scanning ? <Loader2 className="h-4 w-4 animate-spin" /> : null}AI Scan</Button></div>
            ...
            <div><div className="mb-2 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Digit distribution</p><span className="text-xs text-slate-500">Click a digit to select it</span></div><div className="grid grid-cols-5 gap-2 sm:grid-cols-10">{digitCounts.map((count, digit) => { const percentage = (count / digitSampleSize) * 100; return <button key={digit} type="button" onClick={() => setSelectedDigit(digit)} className={`rounded-xl border p-2 text-center transition ${selectedDigit === digit ? 'border-cyan-300 bg-cyan-500/20 shadow-[0_0_18px_rgba(34,211,238,0.15)]' : 'border-slate-700 bg-[#091221] hover:border-cyan-500'}`}><span className={`mx-auto flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-black ${selectedDigit === digit ? 'border-cyan-300 bg-cyan-500 text-slate-950' : 'border-slate-700 text-white'}`}>{digit}</span><span className="mt-1 block text-[11px] text-slate-400">{percentage.toFixed(1)}%</span></button>; })}</div></div>
            ...
            <div className="rounded-xl border border-cyan-900 bg-cyan-950/30 p-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">Lowest-frequency scan</p><p className="mt-3 text-lg font-bold text-white">{scan?.targetDigit == null ? 'Scan the live sample' : `Suggested digit ${scan.targetDigit}`}</p><p className="mt-2 text-sm text-slate-300">{scan?.reason || 'AI suggests the least-occurring digit. Select any digit above to execute manually.'}</p>{scan ? <div className="mt-4 grid grid-cols-3 gap-2 text-xs"><div><span className="text-slate-400">Payout</span><strong className="mt-1 block text-white">{scan.payoutRate == null ? '-' : `${scan.payoutRate}%`}</strong></div><div><span className="text-slate-400">Sample</span><strong className="mt-1 block text-white">{scan.sampleSize}</strong></div><div><span className="text-slate-400">Recent</span><strong className="mt-1 block text-white">{scan.targetOccurrences}</strong></div></div> : null}</div>
            <div className="grid gap-3 sm:grid-cols-2"><Button disabled={trading} onClick={() => void placeTrade('DIGITMATCH')} className="h-12 bg-emerald-600 text-white hover:bg-emerald-700">MATCHES {selectedDigit} x{totalTrades} {matchProposal?.payoutRate == null ? '' : `${matchProposal.payoutRate}% payout`}</Button><Button disabled={trading} onClick={() => void placeTrade('DIGITDIFF')} className="h-12 bg-rose-600 text-white hover:bg-rose-700">DIFFERS {selectedDigit} x{totalTrades} {diffProposal?.payoutRate == null ? '' : `${diffProposal.payoutRate}% payout`}</Button></div>

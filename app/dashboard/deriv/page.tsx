...
import { AlertTriangle, Crown, Info, Loader2, PanelRightOpen, RefreshCcw, Target, Wifi, WifiOff, X, Zap } from 'lucide-react';
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pb-1">
                    ...
                    <Button variant="outline" onClick={() => void analyzeChart(true)} disabled={analyzing || candles.length < 50} className="h-10 border-slate-700 bg-slate-900/70 px-3 text-sm text-slate-100 hover:bg-slate-800 sm:h-11 sm:px-4">
                      <RefreshCcw className={`mr-1.5 h-4 w-4 ${analyzing ? 'animate-spin' : ''}`} />
                      Re-analyze
                    </Button>
                    <Button onClick={() => void analyzeChart(false)} disabled={analyzing || candles.length < 50} className="h-10 bg-blue-500 px-3 text-sm text-white hover:bg-blue-600 sm:h-11 sm:px-4">
